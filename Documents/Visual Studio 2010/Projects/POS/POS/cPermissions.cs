using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Data.SqlClient;
using System.Configuration;
using System.Data;
using System.Windows.Forms;

namespace POS
{
    class cPermissions
    {
        private uint mPermissionLevelID;
		private uint mPermissionLevel;
        private string mInfo;
        private string mDateAdded;

        private SqlCommand cmd;
        private SqlParameter param;
        private SqlConnection con;

        private string constr = new cConstr().Constr;

        public cPermissions()
        {
            //constr = "Data Source=AKOSUAPC;Initial Catalog=Ako;Integrated Security=True";

        }
        public cPermissions(uint PermissionLevelID, uint PermissionLevel, string Info, string DateAdded)
        {
            mPermissionLevelID = PermissionLevelID;
            mInfo = Info;
            mPermissionLevel = PermissionLevel;
        }

        public uint PermissionLevelID
        {
            get { return mPermissionLevelID; }
            set { mPermissionLevelID = value; }
        }

        public uint PermissionLevel
        {
            get { return mPermissionLevel; }
            set { mPermissionLevel = value; }
        }

        public string Info
        {
            get { return mInfo; }
            set { mInfo = value; }
        }

        public string DateAdded
        {
            get { return mDateAdded; }
            set { mDateAdded = value; }
        }
        

        public void openConnection()
        {
            cmd = new SqlCommand();
            con = new SqlConnection();
            con.ConnectionString = constr;
            cmd.Connection = con;
            con.Open();
            cmd.CommandType = CommandType.StoredProcedure;
        }

        //add values to a column - strings (varchar)
        private void query(string parameterName, string parameterValue)
        {
            param = new SqlParameter();
            param.ParameterName = parameterName;
            param.SqlDbType = SqlDbType.VarChar;
            param.Direction = ParameterDirection.Input;
            param.Value = parameterValue;
            cmd.Parameters.Add(param);
        }
        //add integers
        private void query(string parameterName, int parameterValue)
        {
            param = new SqlParameter();
            param.ParameterName = parameterName;
            param.SqlDbType = SqlDbType.Int;
            param.Direction = ParameterDirection.Input;
            param.Value = parameterValue;
            cmd.Parameters.Add(param);
        }
        //add decimals
        private void query(string parameterName, decimal parameterValue)
        {
            param = new SqlParameter();
            param.ParameterName = parameterName;
            param.SqlDbType = SqlDbType.Decimal;
            param.Direction = ParameterDirection.Input;
            param.Value = parameterValue;
            cmd.Parameters.Add(param);
        }

        public bool saveRecord()
        {
            openConnection();
            cmd.CommandText = "prc_PrmssionLvlSave";

            query("@PermissionLevelID", PermissionLevelID);
            query("@PermissionLevel", PermissionLevel);
            query("@Info", Info);
            query("@DateAdded", DateAdded);

            try
            {
                cmd.ExecuteNonQuery();
                return true;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                return false;
                throw ex;
            }
            finally
            {
                con.Dispose();
                con.Close();
            }
        }

        public bool prmssionLvlDelete()
        {
            openConnection();
            cmd.CommandText = "prc_PrmssionLvlDelete";

            query("@PermissionLevel", PermissionLevel);

            try
            {
                cmd.ExecuteNonQuery();
                return true;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                return false;
                throw ex;
            }
            finally
            {
                con.Dispose();
                con.Close();
            }
        }

        public bool prmssionLvlCheck()
        {
            openConnection();
            cmd.CommandText = "prc_prmssionLvlCheck";

            query("@PermissionLevel", PermissionLevel);

            SqlDataReader dr = cmd.ExecuteReader();
            try
            {
                while (dr.Read())
                {
                    if (dr.GetSqlValue(0).ToString() == "1")
                        return true;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                return false;
                throw ex;
            }
            finally
            {
                con.Dispose();
                con.Close();
            }
            return false;
        }

        public DataSet prmissionLvlGet()
        {
            openConnection();
            cmd.CommandText = "prc_prmssionLvlGet";

            query("PermissionLevelID", PermissionLevelID);

            DataSet ds = new DataSet();
            try
            {
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                //------------------------------------------------------------
                da.Fill(ds, "Meals");
                return ds;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                throw ex;
            }
            finally
            {
                ds.Dispose();
                con.Dispose();
                con.Close();
            }
        }

        public DataSet specificMealGet(string Category)
        {
            openConnection();
            cmd.CommandText = "prc_SpecificMealGet";

            query("@Category", Category);

            DataSet ds = new DataSet();
            try
            {
                SqlDataAdapter da = new SqlDataAdapter(cmd);

                da.Fill(ds, "Meals");
                return ds;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                throw ex;
            }
            finally
            {
                ds.Dispose();
                con.Dispose();
                con.Close();
            }
        }

        public DataSet specQtyTypeGet(string QtyType)
        {
            openConnection();
            cmd.CommandText = "prc_SpecQtyTypeGet";

            query("@QuanType", QtyType);

            DataSet ds = new DataSet();
            try
            {
                SqlDataAdapter da = new SqlDataAdapter(cmd);

                da.Fill(ds, "Meals");
                return ds;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                throw ex;
            }
            finally
            {
                ds.Dispose();
                con.Dispose();
                con.Close();
            }
        }



    }
}
